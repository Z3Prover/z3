...
def mk_java(java_src, java_dir, package_name):
    ...
    java_native.write('import java.io.*;\n')
    java_native.write('import java.nio.file.*;\n')
    java_native.write('import java.util.Locale;\n')
    ...
    java_native.write('          loadBundledLibraries(ex, ex2);\n')
    java_native.write('        }\n')
    java_native.write('      }\n')
    java_native.write('    }\n')
    java_native.write('  }\n\n')
    java_native.write('  private static void loadBundledLibraries(UnsatisfiedLinkError ex, UnsatisfiedLinkError ex2) {\n')
    java_native.write('    try {\n')
    java_native.write('      Path directory = Files.createTempDirectory("z3-native-");\n')
    java_native.write('      directory.toFile().deleteOnExit();\n')
    java_native.write('      String platform = bundledPlatform();\n')
    java_native.write('      Path z3 = extractBundledLibrary(directory, platform, bundledLibraryNames("z3"));\n')
    java_native.write('      Path z3java = extractBundledLibrary(directory, platform, bundledLibraryNames("z3java"));\n')
    java_native.write('      if (z3 == null || z3java == null)\n')
    java_native.write('        throw new FileNotFoundException("No bundled native libraries for " + platform);\n')
    java_native.write('      System.load(z3.toAbsolutePath().toString());\n')
    java_native.write('      System.load(z3java.toAbsolutePath().toString());\n')
    java_native.write('    } catch (IOException | UnsatisfiedLinkError bundledEx) {\n')
    java_native.write('      UnsatisfiedLinkError error = new UnsatisfiedLinkError(\n')
    ...
    java_native.write('        + "Tried bundled native libraries: " + bundledEx.getMessage() + ". "\n')
    ...
    java_native.write('      error.initCause(bundledEx);\n')
    java_native.write('      throw error;\n')
    java_native.write('    }\n')
    java_native.write('  }\n\n')
    java_native.write('  private static String bundledPlatform() {\n')
    java_native.write('    String os = System.getProperty("os.name").toLowerCase(Locale.ROOT);\n')
    java_native.write('    String arch = System.getProperty("os.arch").toLowerCase(Locale.ROOT);\n')
    java_native.write('    String osPart;\n')
    java_native.write('    if (os.contains("win"))\n')
    java_native.write('      osPart = "win";\n')
    java_native.write('    else if (os.contains("mac") || os.contains("darwin"))\n')
    java_native.write('      osPart = "osx";\n')
    java_native.write('    else if (os.contains("linux"))\n')
    java_native.write('      osPart = "linux";\n')
    java_native.write('    else\n')
    java_native.write('      osPart = os.replaceAll("[^a-z0-9]+", "");\n')
    java_native.write('    String archPart;\n')
    java_native.write('    if (arch.equals("x86_64") || arch.equals("amd64"))\n')
    java_native.write('      archPart = "x64";\n')
    java_native.write('    else if (arch.equals("x86") || arch.equals("i386") || arch.equals("i486") || arch.equals("i586") || arch.equals("i686"))\n')
    java_native.write('      archPart = "x86";\n')
    java_native.write('    else if (arch.equals("aarch64") || arch.equals("arm64"))\n')
    java_native.write('      archPart = "arm64";\n')
    java_native.write('    else\n')
    java_native.write('      archPart = arch.replaceAll("[^a-z0-9]+", "");\n')
    java_native.write('    return osPart + "-" + archPart;\n')
    java_native.write('  }\n\n')
    java_native.write('  private static String[] bundledLibraryNames(String name) {\n')
    java_native.write('    String mapped = System.mapLibraryName(name);\n')
    java_native.write('    if (System.getProperty("os.name").toLowerCase(Locale.ROOT).contains("win"))\n')
    java_native.write('      return new String[] { mapped, "lib" + mapped };\n')
    java_native.write('    return new String[] { mapped };\n')
    java_native.write('  }\n\n')
    java_native.write('  private static Path extractBundledLibrary(Path directory, String platform, String[] names) throws IOException {\n')
    java_native.write('    for (String name : names) {\n')
    java_native.write('      String resource = "native/" + platform + "/" + name;\n')
    java_native.write('      try (InputStream input = Native.class.getResourceAsStream(resource)) {\n')
    java_native.write('        if (input == null)\n')
    java_native.write('          continue;\n')
    java_native.write('        Path target = directory.resolve(name);\n')
    java_native.write('        Files.copy(input, target, StandardCopyOption.REPLACE_EXISTING);\n')
    java_native.write('        target.toFile().deleteOnExit();\n')
    java_native.write('        return target;\n')
    java_native.write('      }\n')
    java_native.write('    }\n')
    java_native.write('    return null;\n')
    java_native.write('  }\n')
    java_native.write("""
