...
DECIDED = {"sat", "unsat"}
LEGACY_STATUSES = ("sat", "unsat", "unknown", "timeout", "error")
STATUSES = LEGACY_STATUSES + ("invalid-model", "protocol-error")
...
PERF_STUDY = {**DEFAULT_STUDY, "smt.nseq.equation_abstraction": True}
PERF_FLIPS = {
    "TTTT": {},
    "FTTT": {"smt.nseq.regex_parikh": False},
    "TFTT": {"smt.nseq.equation_abstraction": False},
    "TTFT": {"smt.nseq.reverse_retry": False},
    "TTTF": {"smt.nseq.regex_factorization_threshold": 0},
}


def validate_perf_matrix(matrix):
    if (set(matrix) != {"schema", "order", "baseline", "common", "configurations"}
            or type(matrix["schema"]) is not int or matrix["schema"] != 3
            or matrix["order"] != "PERF"):
        raise ValueError("schema 3 requires the explicit PERF order and canonical fields")
    if matrix["baseline"] != "TTTT" or matrix["common"] != PERF_STUDY:
        raise ValueError("schema 3 requires the frozen TTTT baseline, including native E=true")
    if any(type(matrix["common"][k]) is not type(v) for k, v in PERF_STUDY.items()):
        raise ValueError("schema 3 baseline types must match; factorization threshold remains UINT")
    entries = matrix["configurations"]
    if not isinstance(entries, list) or len(entries) != 5:
        raise ValueError("schema 3 requires exactly five PERF configurations")
    if [e.get("name") for e in entries if isinstance(e, dict)] != list(PERF_FLIPS):
        raise ValueError("schema 3 requires TTTT, FTTT, TFTT, TTFT, TTTF in PERF order")
    for entry in entries:
        expected = PERF_FLIPS[entry["name"]]
        if set(entry) != {"name", "overrides"} or entry["overrides"] != expected:
            raise ValueError("schema 3 requires exactly the declared single PERF flip")
        if any(type(entry["overrides"][k]) is not type(v) for k, v in expected.items()):
            raise ValueError("schema 3 flip types must match; F labels map to numeric 1/0")


def validate_source_defaults(source, spec):
    """Publication guard, separate from unit tests of the portable JSON contract."""
    if spec["schema"] not in (2, 3):
        raise ValueError("source-default validation requires a default-based study")
    text = (Path(source) / "src" / "params" / "smt_params_helper.pyg").read_text()
    declared = {name: (kind, value) for name, kind, value in re.findall(
        r"\('(nseq\.[^']+)', (BOOL|UINT), (True|False|\d+),", text)}
    for name, value in spec["common"].items():
        if not name.startswith("smt.nseq."):
            continue
        expected = ("BOOL" if isinstance(value, bool) else "UINT", str(value))
        if declared.get(name[4:]) != expected:
            raise ValueError(f"source default/type mismatch for {name}: expected {expected}")
...
def record_result_evidence(root):
    write_json(root / "evidence.json", {
        p.relative_to(root).as_posix(): digest(p.read_bytes())
        for p in sorted(root.rglob("*")) if p.is_file() and p != root / "evidence.json"})


def verify_result_evidence(root):
    hashes = json.loads((root / "evidence.json").read_text())
    actual = {p.relative_to(root).as_posix() for p in root.rglob("*")
              if p.is_file() and p != root / "evidence.json"}
    if set(hashes) != actual:
        raise ValueError("result evidence contains missing or unexpected files")
    for relative, expected in hashes.items():
        path = root / relative
        if (not path.resolve().is_relative_to(root.resolve()) or path.is_symlink()
                or digest(path.read_bytes()) != expected):
            raise ValueError("result evidence hash mismatch: " + relative)
...
def configurations(path=CONFIG):
    ...
    if not isinstance(spec, dict) or spec.get("schema") not in (1, 2, 3):
        raise ValueError("requires schema 1 or 2 (historical), or 3 (PERF)")
    if spec["schema"] == 2:
        validate_default_matrix(spec)
    elif spec["schema"] == 3:
        validate_perf_matrix(spec)
    result = {}
...
def parse_result(stdout, stderr, returncode, killed=False, schema=1):
    ...
    model_error = re.search(
        r"\binvalid model\b|\bmodel[- ]validation (?:failed|failure|error)\b|"
        r"\bfailed (?:to validate (?:the |a )?model|model[- ]validation)\b", combined, re.I)
    if model_error and schema == 3:
        return "invalid-model", "model validation rejected the answer", verdicts
    if schema == 3 and len(verdicts) > 1:
        return "protocol-error", "multiple answers for a single-query input", verdicts
...
def execute(argv, timeout, prefix, input_data=None, schema=1):
    ...
    status, reason, verdicts = parse_result(stdout, stderr, code, killed, schema)
    stats = {}
    ...
    if not killed and elapsed > timeout and status not in ("error", "invalid-model", "protocol-error"):
        row.update(status="timeout", reason="completed after total wall deadline")
    write_json(prefix.with_suffix(".json"), row)
    return row


def summarize(rows, manifest, configs, baseline, timeout, expected_runs=None, schema=1):
    statuses = STATUSES if schema == 3 else LEGACY_STATUSES
    counts = {c: dict.fromkeys(statuses, 0) for c in configs}
    strata = defaultdict(Counter)
    by_problem = defaultdict(dict)
    issues, execution_errors = [], []
    for row in rows:
        ...
        if row["status"] == "error":
            detail = {"id": row["id"], "config": row["config"], "error": row["reason"]}
            execution_errors.append(detail)
            if schema != 3:
                issues.append(detail)
        if row["status"] in ("invalid-model", "protocol-error"):
            issues.append({"id": row["id"], "config": row["config"], "error": row["reason"]})
        if (schema != 3 or row["status"] in DECIDED) and row["expected"] in DECIDED and any(
    ...
    for row in rows:
        if schema != 3 or row["status"] in DECIDED:
    ...
    if schema == 3 and len({(r["id"], r["repeat"], r["config"]) for r in rows}) != len(rows):
        issues.append({"error": "duplicate measurements"})
    return {"counts": counts, "paired": pairs, "issues": issues,
            "execution_errors": execution_errors,
            "results_policy": {"schema": schema, "execution_errors_fatal": schema != 3,
                               "native_zero_errors_is_diagnostic": schema == 3},
            ...
            "timeout_treatment": "Joint-decision ratios exclude ALL undecided/error runs. "
                                 ...
                                 + ("Execution errors are tolerated unsuccessful outcomes; invalid models, "
                                    "contradictions and integrity failures still reject comparison. "
                                    if schema == 3 else
                                    "Any error/contradiction invalidates comparison. ")
                                 + "No invented solve times."}
...
def run(args):
    ...
    if spec["schema"] == 3:
        env["launch_prefix"] = [solver, *limits]
            ...
            for i, entry in enumerate(manifest["selected"]):
                ...
                for name in names:
                    ...
                    row = execute([solver, *limits, *configs[name],
                                   ...
                                   args.timeout, prefix, schema=spec["schema"])
                    ...
                    writer.writerow(row)
                    csv_file.flush()
                    summary = summarize(rows, manifest, configs, spec["baseline"], args.timeout,
                                        expected_runs, schema=spec["schema"])
    ...
    if summary["execution_errors"]:
        print(f"WARNING: {len(summary['execution_errors'])} execution errors retained as "
              "unsuccessful outcomes; native zero errors was expected, not required.", flush=True)
...
def merge_results(args):
    ...
    for root in roots:
        ...
        for row in local_rows:
            ...
            if spec["schema"] == 3:
                for directory, field in (("inputs", "input_sha256"), ("original", "sha256")):
                    if digest((root / directory / (row["id"] + ".smt2")).read_bytes()) != entry[field]:
                        raise ValueError(f"input evidence mismatch: {key}")
                argv = row.get("argv")
                if (not isinstance(argv, list) or not argv or
                        argv[:-1] != env["launch_prefix"] + configs[row["config"]] or
                        not argv[-1].replace("\\", "/").endswith("/inputs/" + row["id"] + ".smt2")):
                    raise ValueError(f"raw command/configuration mismatch: {key}")
                prefix = root / "raw" / row["id"] / f"{row['repeat']:03d}-{row['config']}"
                execution = json.loads(prefix.with_suffix(".json").read_text())
                if any(row.get(k) != v for k, v in execution.items()):
                    raise ValueError(f"raw execution record mismatch: {key}")
                status, reason, verdicts = parse_result(
                    prefix.with_suffix(".stdout").read_bytes(),
                    prefix.with_suffix(".stderr").read_bytes(),
                    row["returncode"], row["killed"], schema=3)
                if not row["killed"] and row["seconds"] > timeout and status not in (
                        "error", "invalid-model", "protocol-error"):
                    status, reason = "timeout", "completed after total wall deadline"
                if (status, reason, verdicts) != (row["status"], row["reason"], row["verdicts"]):
                    raise ValueError(f"raw classification mismatch: {key}")
            seen.add(key)
    ...
    summary = summarize(rows, manifest, configs, baseline, timeout,
                        len(selected) * repeats * len(configs), schema=spec["schema"])
...
def main(argv=None):
    ...
    p = sub.add_parser("validate-source", help="reject mismatched native defaults/types before building")
    p.add_argument("--source", type=Path, required=True)
    p.add_argument("--config", type=Path, default=CONFIG)
    p.set_defaults(action=lambda a: validate_source_defaults(a.source, configurations(a.config)[0]))
