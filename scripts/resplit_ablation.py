...
def record_build(args):
    ...
    record = {"schema": 1, "source": str(root), "source_identity": identity,
              ...
              "version": version.stdout.decode(errors="replace"), "build_command": args.build_command,
    ...
    for path in files:
        ...
        try:
            ...
            if key in seen:
                ...
                if original["expected"] not in DECIDED and row["expected"] in DECIDED:
            ...
            rejected.append(row)
    # Same evenly spaced selection as noodler_bench.py, but AFTER explicit
    # single-query validation and token-level deduplication.
    if args.limit:
...
def merge_results(args):
    ...
    write_json(out / "sources.json", {
        ...
        "order": "Configuration blocks; not the locally interleaved nine-arm execution.",
    })
    with (out / "runs.jsonl").open("w", encoding="utf-8") as raw, (
            out / "runs.csv").open("w", newline="", encoding="utf-8") as csv_file:
...
def main(argv=None):
    ...
    p.add_argument("--only", help="comma-separated configuration names; omit for all nine")
    ...
    p.add_argument("--config", type=Path, default=CONFIG)
    p.add_argument("--name", default="all-on")
    p.set_defaults(action=lambda a: print(json.dumps(configurations(a.config)[1][a.name])))
