...
class seq_monadic_test {
    ...
    // Pull every branch of the conjunction `mems`.  When `itl` is given, an unrelated
    // decision is run between two pulls, which takes the search state away: the iterator
    // has to notice and stop rather than carry on with somebody else's stack.
    ...
    // The branches of `mems` have to come out one at a time, without repetition, and their
    // number has to agree with the verdict check() gives on the same conjunction.  An
    // enumeration interrupted by another decision has to end in a give-up, and what it
    // did report before that has to be a prefix of the undisturbed run.
    ...
    // A search that runs out of budget has to report a give-up, never a verdict: an
    // abandoned tree is unexplored, so unwinding it neither proves nor refutes anything.
