...
static void test_nseq_parikh_expanded_length_target() {
    ast_manager m;
    reg_decl_plugins(m);
    seq_util u(m);
    euf::egraph eg(m);
    ...
    seq::context_solver_i context_solver;
    seq::nielsen_graph ng(sg, solver, context_solver);
    seq::parikh shared(m, seq::parikh::config());
    expr_ref x(m.mk_const("length.x", u.str.mk_string_sort()), m);
    expr_ref ab(u.str.mk_string(zstring("ab")), m);
    expr_ref subject(u.str.mk_concat(x, u.str.mk_concat(ab, x)), m);
    expr_ref regex(u.re.mk_star(u.re.mk_to_re(ab)), m);
    auto* snode = sg.mk(subject);
    expr_ref target = ng.compute_length_expr(snode);
    expr_ref raw_length(u.str.mk_length(subject), m);
    ENSURE(target != raw_length);

    // The current consolidated encoder still owns the flow constraints and
    // stable skolem keys. The nseq adapter must preserve the expanded target
    // expected by its root arithmetic solver, rather than assert it is str.len.
    expr_ref_vector raw(m);
    ENSURE(shared.encode_length_set(subject, regex, raw));
    vector<seq::constraint> expanded;
    ENSURE(ng.parikh().encode_length_set(subject, regex, target, nullptr, expanded));
    ENSURE(raw.size() == expanded.size() && !raw.empty());
    for (unsigned i = 0; i + 1 < raw.size(); ++i)
        ENSURE(raw[i] == expanded[i].fml);
    ENSURE(m.is_eq(raw.back()) && m.is_eq(expanded.back().fml));
    ENSURE(to_app(raw.back())->get_arg(0) == raw_length);
    ENSURE(to_app(expanded.back().fml)->get_arg(0) == target);
    ENSURE(to_app(raw.back())->get_arg(1) == to_app(expanded.back().fml)->get_arg(1));

    // Exercise the real caller as well; P=false still suppresses only this
    // optional strengthening, not the separately tested exact length checks.
    ng.add_str_mem(snode, sg.mk(regex));
    vector<seq::length_constraint> lengths;
    ng.generate_length_constraints(lengths);
    ENSURE(!lengths.empty());
...
void tst_nseq_ablation() {
    ...
    std::cout << "  consolidated Parikh expanded length target" << std::endl;
    test_nseq_parikh_expanded_length_target();
