...
// ---------------------------------------------------------------------------
// Factorization performance and correctness tests
// ---------------------------------------------------------------------------

// Test: multivariate quadratic factorization via factor_2_sqf_pp
// Verifies that degree-2 multivariate polynomials with perfect-square discriminants
// are correctly decomposed, and that the product of factors equals the original.
static void tst_factor_quadratic_multivariate() {
    std::cout << "\n=== tst_factor_quadratic_multivariate ===\n";
    params_ref ps;
    reslimit rlim;
    nlsat::solver s(rlim, ps, false);
    nlsat::pmanager& pm = s.pm();
    ...
    // p = x0^2 - x1^2 = (x0 - x1)(x0 + x1), quadratic in x0
    polynomial_ref p(pm);
    p = (_x0 ^ 2) - (_x1 ^ 2);
    ...
    factor(p, fs);
    sw.stop();
    std::cout << "  x0^2 - x1^2: " << fs.distinct_factors() << " factors, time=" << sw.get_seconds() << "s\n";
    VERIFY(fs.distinct_factors() == 2);

    polynomial_ref reconstructed(pm);
    fs.multiply(reconstructed);
    VERIFY(eq(reconstructed, p));

    // p2 = x0^2 + 2*x0*x1 + x1^2 = (x0 + x1)^2, has repeated factor
    polynomial_ref p2(pm);
    p2 = (_x0 ^ 2) + 2 * _x0 * _x1 + (_x1 ^ 2);

    polynomial::factors fs2(pm);
    sw.reset();
    sw.start();
    factor(p2, fs2);
    sw.stop();
    std::cout << "  (x0+x1)^2: " << fs2.distinct_factors() << " factors, time=" << sw.get_seconds() << "s\n";
    VERIFY(fs2.distinct_factors() == 1);
    VERIFY(fs2.get_degree(0) == 2);

    polynomial_ref reconstructed2(pm);
    fs2.multiply(reconstructed2);
    VERIFY(eq(reconstructed2, p2));

    std::cout << "  PASS\n";
}

// Test: factorization through the polynomial cache
// Verifies that caching works: factoring the same polynomial twice should be fast.
static void tst_factor_cache_performance() {
    std::cout << "\n=== tst_factor_cache_performance ===\n";
    params_ref ps;
    reslimit rlim;
    nlsat::solver s(rlim, ps, false);
    nlsat::pmanager& pm = s.pm();
    ...
    // Create a moderately complex polynomial: (x0^2 - x1^2) * (x0 + 1)
    polynomial_ref p(pm);
    p = ((_x0 ^ 2) - (_x1 ^ 2)) * (_x0 + 1);

    // First factorization (cache miss)
    polynomial_ref_vector fs1(pm);
    polynomial_ref p_ref(pm);
    p_ref = p;
    stopwatch sw;
    sw.start();
    nlsat::factor(p_ref, cache, fs1);
    sw.stop();
    double first_time = sw.get_seconds();
    std::cout << "  First factorization: " << fs1.size() << " factors, time=" << first_time << "s\n";
    VERIFY(fs1.size() >= 1);

    // Second factorization (cache hit)
    polynomial_ref_vector fs2(pm);
    p_ref = p;
    sw.reset();
    sw.start();
    nlsat::factor(p_ref, cache, fs2);
    sw.stop();
    double second_time = sw.get_seconds();
    std::cout << "  Cached factorization: " << fs2.size() << " factors, time=" << second_time << "s\n";

    // Should get same number of factors
    VERIFY(fs1.size() == fs2.size());

    // Verify factor correctness: product of factors should divide the original
    for (unsigned i = 0; i < fs1.size(); ++i) {
        VERIFY(!pm.is_zero(fs1.get(i)));
        VERIFY(!pm.is_const(fs1.get(i)));
    }

    std::cout << "  PASS\n";
}

// Test: factorization of high-degree multivariate polynomial (degree > 2 path)
// Tests the factor_n_sqf_pp stub and verifies reconstruction correctness.
static void tst_factor_high_degree_multivariate() {
    std::cout << "\n=== tst_factor_high_degree_multivariate ===\n";
    params_ref ps;
    reslimit rlim;
    nlsat::solver s(rlim, ps, false);
    nlsat::pmanager& pm = s.pm();
    ...
    // p = (x0 - x1)*(x0 - 2*x1)*(x0 - 3*x1) -- degree 3 in x0 with x1 coefficients
    polynomial_ref p(pm);
    p = (_x0 - _x1) * (_x0 - 2 * _x1) * (_x0 - 3 * _x1);
    ...
    factor(p, fs);
    sw.stop();
    std::cout << "  (x0-x1)(x0-2x1)(x0-3x1): " << fs.distinct_factors() << " factors, time="
              << sw.get_seconds() << "s\n";

    // Product of factors must equal original
    polynomial_ref reconstructed(pm);
    fs.multiply(reconstructed);
    VERIFY(eq(reconstructed, p));

    // Higher degree: degree 4 in x0
    polynomial_ref p2(pm);
    p2 = (_x0 - _x1) * (_x0 + _x1) * ((_x0 ^ 2) + (_x1 ^ 2));
    polynomial::factors fs2(pm);
    sw.reset();
    sw.start();
    factor(p2, fs2);
    sw.stop();
    std::cout << "  (x0-x1)(x0+x1)(x0^2+x1^2): " << fs2.distinct_factors() << " factors, time="
              << sw.get_seconds() << "s\n";
...
// Test: factorization of a polynomial with many terms (stress test)
// Creates a polynomial with large coefficients and many monomials to test performance.
static void tst_factor_many_terms() {
    std::cout << "\n=== tst_factor_many_terms ===\n";
    params_ref ps;
    reslimit rlim;
    nlsat::solver s(rlim, ps, false);
    nlsat::pmanager& pm = s.pm();
    ...
    // Build product of several linear factors in 3 variables:
    // (x0 + x1 + x2 + 1) * (x0 - x1 + 2) * (x1 + x2 - 3) * (x0 + 2*x2 + 5)
    polynomial_ref f1(pm), f2(pm), f3(pm), f4(pm);
    f1 = _x0 + _x1 + _x2 + 1;
    f2 = _x0 - _x1 + 2;
    f3 = _x1 + _x2 - 3;
    f4 = _x0 + 2 * _x2 + 5;
    ...
    std::cout << "  Polynomial has degree " << pm.total_degree(p) << " with " << pm.size(p) << " monomials\n";
    ...
    factor(p, fs);
    sw.stop();
    std::cout << "  Product of 4 linear factors (3 vars): " << fs.distinct_factors() << " factors, time="
              << sw.get_seconds() << "s\n";
    VERIFY(sw.get_seconds() < 5.0);  // Should complete in reasonable time
...
// Test: factorization with the nlsat::factor wrapper (same path as solver uses)
// This tests the full pipeline: polynomial -> cache -> factor -> distinct factors.
static void tst_factor_nlsat_wrapper() {
    std::cout << "\n=== tst_factor_nlsat_wrapper ===\n";
    params_ref ps;
    reslimit rlim;
    nlsat::solver s(rlim, ps, false);
    nlsat::pmanager& pm = s.pm();
    ...
    // Test 1: univariate quadratic through the wrapper
    polynomial_ref p1(pm);
    p1 = (_x0 ^ 2) - 1;  // (x0-1)(x0+1)
    polynomial_ref_vector fs1(pm);
    nlsat::factor(p1, cache, fs1);
    std::cout << "  x0^2 - 1: " << fs1.size() << " factors\n";
    VERIFY(fs1.size() == 2);

    // Test 2: multivariate quadratic through the wrapper  
    polynomial_ref p2(pm);
    p2 = (_x0 ^ 2) - (_x1 ^ 2);  // (x0-x1)(x0+x1)
    polynomial_ref_vector fs2(pm);
    nlsat::factor(p2, cache, fs2);
    std::cout << "  x0^2 - x1^2: " << fs2.size() << " factors\n";
    VERIFY(fs2.size() == 2);

    // Test 3: constant polynomial should have 0 factors
    polynomial_ref p3(pm);
    p3 = pm.mk_const(rational(42));
    polynomial_ref_vector fs3(pm);
    nlsat::factor(p3, cache, fs3);
    std::cout << "  constant 42: " << fs3.size() << " factors\n";
    VERIFY(fs3.size() == 0);

    // Test 4: linear polynomial - irreducible
    polynomial_ref p4(pm);
    p4 = _x0 + _x1 + 1;
    polynomial_ref_vector fs4(pm);
    nlsat::factor(p4, cache, fs4);
    std::cout << "  x0 + x1 + 1: " << fs4.size() << " factors\n";
    VERIFY(fs4.size() == 1);

    std::cout << "  PASS\n";
}

// Test: factorization of discriminant-like polynomials
// Discriminants arise naturally in the solver when projecting quadratic constraints.
// Tests the b^2 - 4ac pattern with multivariate coefficients.
static void tst_factor_discriminant() {
    std::cout << "\n=== tst_factor_discriminant ===\n";
    params_ref ps;
    reslimit rlim;
    nlsat::solver s(rlim, ps, false);
    nlsat::pmanager& pm = s.pm();
    ...
    // Polynomial: a*x2^2 + b*x2 + c where a=x0, b=x1, c=1
    // Discriminant is x1^2 - 4*x0
    ...
    factor(disc, fs);
    sw.stop();
    std::cout << "  disc = x1^2 - 4*x0: " << fs.distinct_factors() << " factors, time=" << sw.get_seconds() << "s\n";
    // This is irreducible (no perfect square root for discriminant)
    ...
    factor(p2, fs2);
    sw.stop();
    std::cout << "  (x2+x1)^2 - x0: " << fs2.distinct_factors() << " factors, time=" << sw.get_seconds() << "s\n";
...
// Test: repeated factorization of many different polynomials (throughput test)
// Simulates a solver that factors many polynomials during projection.
static void tst_factor_throughput() {
    std::cout << "\n=== tst_factor_throughput ===\n";
    params_ref ps;
    reslimit rlim;
    nlsat::solver s(rlim, ps, false);
    nlsat::pmanager& pm = s.pm();
    ...
    for (unsigned i = 0; i < num_polys; ++i) {
        ...
        polynomial_ref_vector fs(pm);
        nlsat::factor(p, cache, fs);
        // Every factored polynomial should have at least one factor
        VERIFY(fs.size() >= 1);
    }

    sw.stop();
    std::cout << "  Factored " << num_polys << " quadratic polynomials in " << sw.get_seconds() << "s\n";
    VERIFY(sw.get_seconds() < 10.0);  // Should be fast

    std::cout << "  PASS\n";
}

// Test: factorization during projection (integration test)
// Sets up an nlsat scenario and checks that factorization + projection
// completes in reasonable time.
static void tst_factor_projection_perf() {
    std::cout << "\n=== tst_factor_projection_perf ===\n";
    params_ref ps;
    reslimit rlim;
    nlsat::solver s(rlim, ps, false);
    anum_manager& am = s.am();
    ...
    // Create a system with polynomials that require factorization during projection:
    // p1 = x2^2 - x0*x1 (quadratic in x2 with multivariate discriminant)
    // p2 = x2 - x0 - x1
    polynomial_ref p1(pm), p2(pm);
    p1 = (_x2 ^ 2) - _x0 * _x1;
    p2 = _x2 - _x0 - _x1;
    ...
    nlsat::scoped_literal_vector lits(s);
    lits.push_back(mk_gt(s, p1.get()));
    lits.push_back(mk_gt(s, p2.get()));
    ...
    sw.start();
    nlsat::scoped_literal_vector result(s);
    ex.project(x2, lits.size(), lits.data(), result);
    sw.stop();

    std::cout << "  Projection eliminated x2 from 2 constraints: time=" << sw.get_seconds() << "s\n";
    std::cout << "  Result has " << result.size() << " literals\n";
    VERIFY(sw.get_seconds() < 5.0);
