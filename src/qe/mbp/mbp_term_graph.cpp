...
bool term_graph::term_lt(term const &t1, term const &t2) {
    ...
    if (t1.get_num_args() == 0 || t2.get_num_args() == 0) {
        if (t1.get_num_args() == t2.get_num_args()) {
            ...
            // Prefer uninterpreted constants over values as equivalence class
            // representatives. Keeping representatives symbolic is required by
            // model-based projection clients such as Spacer: substituting a
            // (non-eliminated) constant by its concrete model value over-grounds
            // the projected lemma and can cause Spacer to repeatedly regenerate
            // and bump an over-specialized lemma until it aborts with
            // "Stuck on a lemma" (see Z3Prover/bench discussion #3420,
            // benchmark iss-5561/bug-2.smt2).
