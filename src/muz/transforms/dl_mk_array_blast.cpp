...
namespace datalog {
    ...
    bool mk_array_blast::ackermanize(rule const& r, expr_ref& body, expr_ref& head) {
        ...
        for (; it1 != end; ++it1) {
            ...
            for (; it2 != end; ++it2) {
                ...
                conjs.push_back(m.mk_implies(m.mk_and(std::span<expr* const>(eqs.data(), eqs.size())), m.mk_eq(v1, v2)));
            }
        }
        body = m.mk_and(std::span<expr* const>(conjs.data(), conjs.size()));        
        m_rewriter(body);   
        return true;
    }
    ...
    bool mk_array_blast::blast(rule& r, rule_set& rules) {
        ...
        body = m.mk_and(std::span<expr* const>(new_conjs.data(), new_conjs.size()));
        head = r.get_head();
        sub(body);
        m_rewriter(body);
        sub(head);
