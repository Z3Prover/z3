...
struct mus::imp {
    ...
    bool is_literal(expr* lit) const {
        expr* l;
        return is_uninterp_const(lit) ||
               (m.is_not(lit, l) && is_uninterp_const(l));
    }
    ...
    lbool get_mus1(expr_ref_vector& mus) {
        ...
        while (!unknown.empty()) {
            IF_VERBOSE(12, verbose_stream()
                               << "(mus reducing core: " << unknown.size()
                               << " new core: " << mus.size() << ")\n";);
    ...
    lbool get_next_mcs(expr_ref_vector& mus, ptr_vector<expr>& unknown,
                       expr*& core_literal) {
        ptr_vector<expr> mss;
        ...
        while (!unknown.empty()) {
            ...
            unknown.pop_back();
            model_ref mdl;
            scoped_append assume_mss(*this, mus,
                                     mss); // current satisfied literals
            scoped_append assume_nmcs(*this, mus,
                                      nmcs); // current non-satisfied literals
            scoped_append assume_lit(*this, mus,
                                     lit); // current unknown literal
            switch (m_solver.check_sat(mus)) {
            case l_true: {
                TRACE("mus", tout << "literal can be satisfied: "
                                  << mk_pp(lit, m) << "\n";);
                mss.push_back(lit);
            ...
            case l_false:
                ...
                if (!min_core_valid || core.size() < min_core.size()) {
                    // The current core is smallest so far, so we get fewer
                    // unknowns from it.
                    min_core = core;
                    min_core_valid = true;
                    min_lit = lit;
    ...
    public:
        ...
        scoped_append(imp& imp, expr_ref_vector& fmls1,
                      expr_ref_vector const& fmls2)
            : m_fmls(fmls1), m_size(fmls1.size()) {
            fmls1.append(fmls2);
        }
        scoped_append(imp& imp, expr_ref_vector& fmls1,
                      ptr_vector<expr> const& fmls2)
            : m_fmls(fmls1), m_size(fmls1.size()) {
