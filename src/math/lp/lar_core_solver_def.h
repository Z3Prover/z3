...
#include <string>
namespace lp {
lar_core_solver::lar_core_solver(lp_settings &settings,
                                 const column_namer &column_names)
    : m_infeasible_sum_sign(0),
      m_r_solver(m_r_A, m_right_sides_dummy, m_r_x, m_r_basis, m_r_nbasis,
                 m_r_heading, m_costs_dummy, m_column_types(),
                 m_r_lower_bounds(), m_r_upper_bounds(), settings,
                 column_names) {}

void lar_core_solver::prefix_r() {

  // m_r_solver.m_b.resize(m_r_solver.m_m());
  if (m_r_solver.m_settings.simplex_strategy() !=
...
void lar_core_solver::fill_not_improvable_zero_sum_from_inf_row() {
  ...
  for (auto &rc : m_r_solver.m_A.m_rows[m_r_solver.m_inf_row_index_for_tableau])
    m_infeasible_linear_combination.push_back(
        std::make_pair(rc.coeff(), rc.var()));
}
...
void lar_core_solver::fill_not_improvable_zero_sum() {
  ...
  // m_costs are expressed by m_d ( additional costs), substructing the latter
  // gives 0
...
void lar_core_solver::solve() {
  ...
  TRACE("find_feas_stats",
        tout << "infeasibles = " << m_r_solver.inf_heap_size()
             << ", int_infs = " << get_number_of_non_ints() << std::endl;);
  if (m_r_solver.current_x_is_feasible() &&
