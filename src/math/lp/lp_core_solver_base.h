...
template <typename T,
          typename X> // X represents the type of the x variable and the bounds
...
public:
  bool current_x_is_feasible() const {
    TRACE(
        "feas",
        if (m_inf_heap.size() > 1) {
          tout << "column " << *m_inf_heap.begin() << " is infeasible"
               << std::endl;
          print_column_info(*m_inf_heap.begin(), tout);
        } else { tout << "x is feasible\n"; });
    return m_inf_heap.empty();
  }
  bool current_x_is_infeasible() const { return m_inf_heap.empty() == false; }

private:
  lpvar_heap m_inf_heap; // the heap of infeasible columns
public:
  const lpvar_heap &inf_heap() const { return m_inf_heap; }
  lpvar_heap &inf_heap() { return m_inf_heap; }
  void inf_heap_increase_size_by_one() {
    m_inf_heap.reserve(m_inf_heap.size() + 1);
  }
  bool inf_heap_contains(unsigned j) const { return m_inf_heap.contains(j); }
  unsigned inf_heap_size() const { return m_inf_heap.size(); }
  indexed_vector<T>
      m_pivot_row;          // this is the real pivot row of the simplex tableu
  ...
  vector<unsigned>
      m_trace_of_basis_change_vector; // the even positions are entering, the
                                      // odd positions are leaving
  ...
  void stop_tracing_basis_changes() { m_tracing_basis_changes = false; }
  ...
  } // it is the length of basis. The matrix m_A has m_m rows and the dimension
    // of the matrix A is m_m
  unsigned m_n() const {
    return m_A.column_count();
  } // the number of columns in the matrix m_A

  lp_core_solver_base(static_matrix<T, X> &A,
                      // vector<X> & b, // the right side vector
                      vector<unsigned> &basis, vector<unsigned> &nbasis,
                      vector<int> &heading, vector<X> &x, vector<T> &costs,
  ...
  vector<unsigned> &non_basis() { return m_nbasis; }

  const vector<unsigned> &non_basis() const { return m_nbasis; }

  void set_status(lp_status status) { m_status = status; }
  lp_status get_status() const { return m_status; }
  void pretty_print(std::ostream &out);

  X get_cost() const { return dot_product(m_costs, m_x); }

  void add_delta_to_entering(unsigned entering, const X &delta);

  const X &get_var_value(unsigned j) const { return m_x[j]; }
  ...
  print_statistics_with_iterations_and_nonzeroes_and_cost_and_check_that_the_time_is_over(
      char const *str, std::ostream &message_stream);

  bool print_statistics_with_cost_and_check_that_the_time_is_over(
      X cost, std::ostream &message_stream);
  ...
  bool reduced_costs_are_correct_tableau() const {
    ...
    for (unsigned j = 0; j < n; j++) {
      if (m_basis_heading[j] >= 0) {
        ...
        if (m_d[j] != d) {
          TRACE("lar_solver",
                tout << "reduced costs are incorrect for column j = " << j
                     << " should be " << d << " but we have m_d[j] = " << m_d[j]
                     << std::endl;);
          return false;
        }
      }
  ...
  bool below_bound(const X &x, const X &bound) const { return x < bound; }

  bool above_bound(const X &x, const X &bound) const { return x > bound; }
  ...
  void change_basis_unconditionally(unsigned entering, unsigned leaving) {
    TRACE("lar_solver", tout << "entering = " << entering
                             << ", leaving = " << leaving << "\n";);
  ...
  std::ostream &print_column_bound_info(unsigned j, std::ostream &out) const {
    out << column_name(j)
        << " type = " << column_type_to_string(m_column_types[j]) << std::endl;
    switch (m_column_types[j]) {
    case column_type::fixed:
    case column_type::boxed:
  ...
  bool column_is_fixed(unsigned j) const {
    return this->m_column_types[j] == column_type::fixed;
  }
  ...
  template <typename K> static void swap(vector<K> &v, unsigned i, unsigned j) {
    auto t = v[i];
    v[i] = v[j];
    v[j] = t;
  }
  ...
  bool column_is_base(unsigned j) const { return m_basis_heading[j] >= 0; }
  ...
  void add_delta_to_x_and_track_feasibility(unsigned j, const X &del) {
    TRACE("lar_solver", tout << "del = " << del << ", was x[" << j
                             << "] = " << m_x[j] << "\n";);
    m_x[j] += del;
  ...
  void insert_column_into_inf_heap(unsigned j) {
    ...
    lp_assert(j < static_cast<unsigned>(m_inf_heap.get_bounds()));
