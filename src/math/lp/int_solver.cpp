...
int_solver::patcher::patcher(int_solver &lia)
    : lia(lia), lra(lia.lra), lrac(lia.lrac) {}
...
bool int_solver::patcher::patch_basic_column(
    unsigned v, row_cell<mpq> const &c) {
  if (v == c.var())
    return false;
...
bool int_solver::patcher::try_patch_column(unsigned v, unsigned j,
                                           mpq const &delta) {
  const auto &A = lra.A_r();
  unsigned make_count = 0, break_count = 0;
  if (delta < 0) {
    if (lia.has_lower(j) &&
        lia.get_value(j) + impq(delta) < lra.get_lower_bound(j))
      return false;
  } else {
    if (lia.has_upper(j) &&
        lia.get_value(j) + impq(delta) > lra.get_upper_bound(j))
      return false;
  }
...
lia_move int_solver::patcher::patch_nbasic_columns() {
  ...
  lp_assert(lia.is_feasible());
  // verbose_stream() << "patch " << m_patch_success << " fails " <<
  // m_patch_fail << " ones " << m_num_ones << " divides " << m_num_divides << "
  // num fixed " << num_fixed << "\n"; lra.display(verbose_stream()); exit(0);
...
void int_solver::patcher::patch_nbasic_column(unsigned j) {
  ...
  TRACE("patch_int", tout << "TARGET j" << j << " -> [";
        if (inf_l) tout << "-oo"; else tout << l; tout << ", ";
        if (inf_u) tout << "oo"; else tout << u; tout << "]";
        tout << ", m: " << m << ", val: " << val
             << ", is_int: " << lra.column_is_int(j) << "\n";);
...
int_solver::int_solver(lar_solver &lar_slv)
    : lra(lar_slv), lrac(lra.m_mpq_lar_core_solver), m_gcd(*this),
      m_patcher(*this), m_number_of_calls(0), m_hnf_cutter(*this),
...
struct check_return_helper {
  ...
  check_return_helper(lar_solver &ls)
      : lra(ls), m_track_pivoted_rows(lra.get_track_pivoted_rows()) {
    lra.set_track_pivoted_rows(false);
  }
  ~check_return_helper() { lra.set_track_pivoted_rows(m_track_pivoted_rows); }
};
...
lia_move int_solver::check(lp::explanation *e) {
  ...
  if (r == lia_move::undef && should_find_cube())
    r = int_cube(*this)();
  if (r == lia_move::undef && should_hnf_cut())
    r = hnf_cut();
  if (r == lia_move::undef && should_gomory_cut())
    r = gomory(*this)();
  if (r == lia_move::undef)
    r = int_branch(*this)();
  return r;
...
bool int_solver::has_inf_int() const { return lra.has_inf_int(); }
...
lp_settings &int_solver::settings() { return lra.settings(); }

const lp_settings &int_solver::settings() const { return lra.settings(); }

bool int_solver::column_is_int(column_index const &j) const {
  return lra.column_is_int(j);
}

bool int_solver::is_real(unsigned j) const { return !column_is_int(j); }

bool int_solver::value_is_int(unsigned j) const {
  return lra.column_value_is_int(j);
}

unsigned int_solver::random() { return settings().random_next(); }
...
unsigned int_solver::column_count() const { return lra.column_count(); }
...
bool int_solver::get_freedom_interval_for_column(unsigned j, bool &inf_l,
                                                 impq &l, bool &inf_u, impq &u,
                                                 mpq &m) {
  if (lrac.m_r_heading[j] >= 0 || is_fixed(j)) // basic or fixed var
    return false;

  TRACE("random_update", display_column(tout, j)
                             << ", is_int = " << column_is_int(j) << "\n";);
  ...
  TRACE("freedom_interval", tout << "freedom variable for:\n";
        tout << lra.get_variable_name(j); tout << "["; if (inf_l) tout << "-oo";
        else tout << l; tout << "; "; if (inf_u) tout << "oo"; else tout << u;
...
bool int_solver::is_feasible() const {
  lp_assert(lrac.m_r_solver.calc_current_x_is_feasible_include_non_basis() ==
            lrac.m_r_solver.current_x_is_feasible());
  return lrac.m_r_solver.current_x_is_feasible();
}

const impq &int_solver::get_value(unsigned j) const { return lrac.m_r_x[j]; }
...
bool int_solver::is_base(unsigned j) const { return lrac.m_r_heading[j] >= 0; }
...
std::ostream &
int_solver::display_row(std::ostream &out,
                        lp::row_strip<rational> const &row) const {
  bool first = true;
...
std::ostream &int_solver::display_row_info(std::ostream &out,
                                           unsigned row_index) const {
  auto &rslv = lrac.m_r_solver;
...
bool int_solver::shift_var(unsigned j, unsigned range) {
  ...
  SASSERT(!inf_l && !inf_u);
  // The shift has to be a multiple of m: let us look for s, such that the shift
  // is m*s. We have new_val = x+m*s <= u, so m*s <= u-x and, finally, s <=
  // floor((u- x)/m) = a The symmetric reasoning gives us s >= ceil((l-x)/m) = b
  ...
  if (!r.is_pos())
    return false;
  TRACE("int_solver", tout << "a = " << a << ", b = " << b << ", r = " << r
                           << ", m = " << m << "\n";);
  if (r < mpq(range))
