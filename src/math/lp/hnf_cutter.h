...
class hnf_cutter {
    ...
    ptr_vector<constraint_dependency>  m_constraints_for_explanation;
...
private:
    ...
    const ptr_vector<constraint_dependency>& constraints_for_explanation() const { return m_constraints_for_explanation; }
    const vector<mpq> & right_sides() const { return m_right_sides; }

    bool is_full() const;

    void clear();
    void add_term(const lar_term* t, const mpq &rs, constraint_dependency* ci, bool upper_bound);
