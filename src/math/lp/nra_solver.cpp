...
struct solver::imp {
    ...
    */
    ...
    // it is the (recursive) max over factors; for a term column it is the
    // (recursive) max over the term's constituent columns (at least 1).
    // For a constraint we take the max over the lhs terms, since the
    // polynomial is a sum of those monomials. Smaller values are preferred
    // to mimic the lowest-degree heuristic in nlsat_solver::check(rvalues, clause).
    unsigned var_expansion_size(lp::lpvar v) {
        if (m_nla_core.emons().is_monic_var(v)) {
            unsigned s = 1;
            for (auto w : m_nla_core.emons()[v].vars())
                s = std::max(s, var_expansion_size(w));
            return s;
        }
        if (lra.column_has_term(v)) {
            unsigned s = 1;
            for (auto const& [w, coeff] : lra.get_term(v).coeffs())
                s = std::max(s, var_expansion_size(w));
            return s;
        }
        return 1;
