...
sort* basic_decl_plugin::join(sort* s1, sort* s2) {
    ...
    if (s2 == m_bool_sort && s1->get_family_id() == arith_family_id)
        return s1;
    throw ast_exception(std::string("Sorts ") + to_string(mk_pp(s1, *m_manager)) + " and " + to_string(mk_pp(s2, *m_manager)) + " are incompatible");
}
    ...
    if (decl->is_associative()) {
        ...
        for (unsigned i = 0; i < num_args; ++i) {
            ...
            if (!compatible_sorts(expected, given)) {
                throw ast_exception(std::string("invalid function application for ") + to_string(decl->get_name()) + ", sort mismatch on argument at position " + std::to_string(i + 1) + ", expected " + to_string(mk_pp(expected, m)) + " but given " + to_string(mk_pp(given, m)));
            }
        }
    }
    else {
...
void ast_manager::check_args(func_decl* f, unsigned n, expr* const* es) {
    for (unsigned i = 0; i < n; ++i) {
        ...
        if (expected_sort != actual_sort) {
            throw ast_exception(std::string("Sort mismatch at argument #") + std::to_string(i + 1) + " for function " + to_string(mk_pp(f, *this)) + " supplied sort is " + to_string(mk_pp(actual_sort, *this)));
        }
    }
}
...
app * ast_manager::mk_app(func_decl * decl, unsigned num_args, expr * const * args) {
    ...
    if (type_error) {
        ...
        throw ast_exception(std::string("Wrong number of arguments (") + std::to_string(num_args) + ") passed to function " + to_string(mk_pp(decl, *this)) + " " + arg_list);
    }
    app * r = nullptr;
