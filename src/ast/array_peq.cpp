...
app_ref mk_peq(expr *e0, expr *e1,
               vector<expr_ref_vector> const &indices, ast_manager &m) {
...
peq::peq(app* p, ast_manager& m):
            ...
            m_decl (p->get_decl (), m), m_peq (p, m), m_eq (m),
            m_arr_u(m), m_name(symbol(PARTIAL_EQ))
{
