...
namespace euf {
    ...
    void seq_plugin::propagate_merge(enode* a, enode* b) {
        ...
        for (enode* n : m_concats) {
            ...
            if (is_str_concat(n, na, nb)) {
                if (is_str_empty(na->get_root()))
                    push_merge(n, nb);
                else if (is_str_empty(nb->get_root()))
                    push_merge(n, na);
            }
            if (is_re_concat(n, na, nb)) {
                if (is_re_epsilon(na->get_root()))
                    push_merge(n, nb);
                else if (is_re_epsilon(nb->get_root()))
                    push_merge(n, na);
                else if (is_re_empty(na->get_root()))
                    push_merge(n, na);
                else if (is_re_empty(nb->get_root()))
                    push_merge(n, nb);
            }
        }

        // Re-simplify concat nodes whose children have become full_seq (.*) after the
        // merge. This catches nullable absorption through nested concats: when
        // concat(v, w) has v nullable and the outer concat gets .* as a sibling,
        // the outer propagate_simplify fires correctly only if re-triggered here.
        for (enode* n : m_concats) {
            enode *na, *nb;
            if (is_re_concat(n, na, nb)) {
    ...
    //
    // 3. Loop merging: concat(v{l1,h1}, v{l2,h2}) = v{l1+l2,h1+h2}
    //    when adjacent loops in a concat chain have congruent bodies.
    //
