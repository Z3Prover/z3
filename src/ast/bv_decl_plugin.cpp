    ...
    if (r != nullptr) {
        ...
        for (unsigned i = 0; i < num_args; ++i) {
            if (args[i]->get_sort() != r->get_domain(i)) {
                m.raise_exception(std::string("Argument ") + to_string(mk_pp(args[i], m)) + " at position " + std::to_string(i) + " has sort " + to_string(mk_pp(args[i]->get_sort(), m)) + " it does not match declaration " + to_string(mk_pp(r, m)));
                return nullptr;
            }
        }
        return r;
