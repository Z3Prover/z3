...
private:
    ...
    // A parked branch: the search state of an `iterator` that is not the one the engine
    // currently serves.  Parking lifts the whole search out of the engine, so any number
    // of enumerations can be suspended across each other and across ordinary
    // solve()/check() calls.  `m_pin` is the parked state's own, because the engine drops
    // its pins (and its caches) at the start of every search, and a branch points at
    // derivative states that nothing else keeps alive.
    struct parked_search {
        vector<vector<atom>>     m_atoms;
        expr_ref_vector          m_regexes;
        ptr_vector<expr>         m_vars;
        obj_map<expr, unsigned>  m_var_idx;
        vector<seq::view_vector> m_groups;
        obj_map<expr, uint64_t>  m_last_occ;
        svector<frame>           m_stack;
        obj_map<expr, seq::view_vector> m_solution;
        expr_ref_vector          m_pin;
        expr*    m_pos_R = nullptr;
        unsigned m_pos_mi = 0, m_pos_i = 0;
        unsigned m_undef_vars = 0;
        bool     m_any_undef = false;
        bool     m_reversed = false;

        parked_search(ast_manager& m) : m_regexes(m), m_pin(m) {}

        void swap(parked_search& o) {
            m_atoms.swap(o.m_atoms);
            m_regexes.swap(o.m_regexes);
            m_vars.swap(o.m_vars);
            std::swap(m_var_idx, o.m_var_idx);
            m_groups.swap(o.m_groups);
            std::swap(m_last_occ, o.m_last_occ);
            m_stack.swap(o.m_stack);
            std::swap(m_solution, o.m_solution);
            m_pin.swap(o.m_pin);
            std::swap(m_pos_R, o.m_pos_R);
            std::swap(m_pos_mi, o.m_pos_mi);
            std::swap(m_pos_i, o.m_pos_i);
            std::swap(m_undef_vars, o.m_undef_vars);
            std::swap(m_any_undef, o.m_any_undef);
            std::swap(m_reversed, o.m_reversed);
        }
    };
    ...
    void swap_search(parked_search& s);

    // Pin every term the parked state refers to, so it survives the engine clearing its
    // own pins.  Takes the new references before dropping the old ones.
    void pin_parked(parked_search& s);

    // Park the iterator the engine currently serves, leaving the search state empty.
    // Every entry point that starts a fresh search calls this first.
    void park_active();

    // Make `it` the served iterator, parking whoever held the engine before.
    void activate(iterator* it);
...
public:
    ...
    //
    // The engine serves one search at a time, but an iterator that loses it PARKS its
    // branch (stack, groups, parsed query, and pins of its own) inside itself and gets it
    // back on the next pull, so any number of iterators can be suspended across each
    // other and across ordinary solve()/check() calls.
    //
