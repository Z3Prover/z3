...
expr_ref seq_split::mk_charclass_re(expr* pred, sort* seq_sort) {
    ...
    if (seq::guard_to_range_predicate(sq, var0, pred, rp))
...
// One level of the sigma rules.  Emits *suspended* split-algebra terms (from_re /
// lcat / rcat / inter / compl) for the subterms instead of recursing.  `mode` is
// irrelevant here: weak vs. strong is decided when `head_normalize` reaches an
// inter / compl node.
