...
#define SMT_PARAMS_HELPER_LIST(UINT_, BOOL_, DOUBLE_, STRING_, SYMBOL_) \
  ...
  BOOL_  (arith_nl_transcendental,                 "arith.nl.transcendental",                 true,                     "delta-check transcendental function applications (sin/cos/tan/asin/acos/atan/sinh/cosh/tanh/asinh/acosh/atanh) registered by theory_lra against the current assignment using floating point evaluation") \
  DOUBLE_(arith_nl_transcendental_tolerance,       "arith.nl.transcendental_tolerance",       0.0000001,                "delta tolerance used when delta-checking transcendental function applications against the current assignment") \
  ...
  SYMBOL_(string_solver,                           "string_solver",                           "seq",                    "solver for string/sequence theories. options are: 'z3str3' (specialized string solver), 'seq' (sequence solver), 'nseq' (stx-search-tree based sequence solver), 'auto' (use static features to choose best solver), 'empty' (a no-op solver that forces an answer unknown if strings were used), 'none' (no solver)") \
  ...
  BOOL_  (seq_eq_approx,                           "seq.eq_approx",                           false,                    "nseq: enable eq_approx_split (c3's opt-in apply_eq_approx)") \
  BOOL_  (seq_fine_wilf,                           "seq.fine_wilf",                           false,                    "nseq: enable power_fine_wilf (c3's opt-in apply_fine_wilf)") \
  BOOL_  (seq_mem_parikh,                          "seq.mem_parikh",                          false,                    "nseq: enable mem_parikh_split (c3's opt-in Parikh feasibility check)") \
  BOOL_  (seq_monadic_leaf,                        "seq.monadic_leaf",                        false,                    "nseq: enable mem_leaf_split, the whole-language monadic decision over the conjunction of plain regex memberships (c3's monadic_leaf); off by default, it regresses the regex benchmarks") \
  ...
  BOOL_  (seq_regex_precheck,                      "seq.regex_precheck",                      true,                     "nseq: incremental single-variable regex-intersection feasibility check (view_witness)") \
  UINT_  (seq_block_compression,                   "seq.block_compression",                   0,                        "nseq: a variable facing a block of single-character tokens is split against the whole block at once; caps the block length (0 = unlimited, 1 = one character at a time)") \
