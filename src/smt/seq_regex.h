...
namespace smt {
    ...
    class seq_regex {
        ...
        */
        ...
        // Is |s| already determined by lengths arithmetic reasons about?  True when |s| is
        // itself internalized, or when every non-constant part of s has an internalized
        // length -- theory_seq derives |xy| = |x| + |y|, so materializing |s| then adds no
        // new length variable, only a sum of live ones.
        bool lengths_are_live(expr* s);
