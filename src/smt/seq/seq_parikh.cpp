...
namespace seq {
    ...
    bool seq_parikh::encode_length_set(expr* str_key, expr* re, expr* len_target, dep_tracker dep, vector<constraint>& out) {
        ...
        // produces.  The root uses compute_length_expr, which expands lengths
        // of concatenations and powers: keep that arithmetic target while the
        // original str_key still determines the stable visit-count skolems.
        expr_ref_vector asserts(m);
        if (!m_pk.encode_length_set(str_key, re, asserts, len_target))
            return false;
        for (expr* e : asserts)
