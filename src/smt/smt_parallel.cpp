    ...
    void parallel::batch_manager::get_cubes(ast_translation& g2l, vector<expr_ref_vector>& cubes) {
        ...
        for (unsigned i = 0; i < std::min(m_max_batch_size / p.num_threads, (unsigned)m_cubes.size()) && !m_cubes.empty(); ++i) {
            if (m_config.m_frugal_deepest_cube_only) {
                // get a random cube from std::map<unsigned, vector<expr_ref_vector>> m_cubes_depth_sets;, i.e. the set at the end of the list THAT IS NOT EMPTY
                // .back() does NOT work since this is not a vector!!
                // i just need to find the set with the greatest depth, i.e. the set associated with the biggest key in the map
