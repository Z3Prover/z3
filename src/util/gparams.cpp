...
struct gparams::imp {
    ...
    void check_registered() {
        ...
        for (auto * r = gparams::g_module_registrations; r; r = r->next) {
            register_module(r->module_name, r->get_descrs);
            if (r->descr)
                register_module_descr(r->module_name, r->descr);
        }
        for (auto * r = gparams::g_global_registrations; r; r = r->next) {
            param_descrs d;
            r->collect(d);
            register_global(d);
        }
...
gparams::module_registration * gparams::g_module_registrations = nullptr;
gparams::global_registration * gparams::g_global_registrations = nullptr;

gparams::module_registration::module_registration(char const * module_name, lazy_descrs_t get_descrs, char const * descr):
    module_name(module_name), get_descrs(get_descrs), descr(descr), next(g_module_registrations) {
    g_module_registrations = this;
}

gparams::global_registration::global_registration(collect_descrs_t collect):
    collect(collect), next(g_global_registrations) {
    g_global_registrations = this;
}
