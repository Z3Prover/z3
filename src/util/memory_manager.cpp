...
memory::initializer_registration * memory::g_initializer_registrations = nullptr;
memory::finalizer_registration * memory::g_finalizer_registrations = nullptr;

memory::initializer_registration::initializer_registration(fn_t fn, int priority):
    fn(fn), priority(priority), next(g_initializer_registrations) {
    g_initializer_registrations = this;
}

memory::finalizer_registration::finalizer_registration(fn_t fn):
    fn(fn), next(g_finalizer_registrations) {
    g_finalizer_registrations = this;
}

void memory::run_initializers() {
    std::vector<initializer_registration const *> v;
    for (auto * r = g_initializer_registrations; r; r = r->next)
        v.push_back(r);
...
void memory::run_finalizers() {
    for (auto * r = g_finalizer_registrations; r; r = r->next)
        r->fn();
}
