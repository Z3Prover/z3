...
def get_version_string():
    ...
    rev = ffi.new('unsigned*')
    Z3_get_version(major, minor, build, rev)
    return "%u.%u.%u" % (major[0], minor[0], build[0])
...
def get_version():
    ...
    rev = ffi.new('unsigned*')
    Z3_get_version(major, minor, build, rev)
    return (major[0], minor[0], build[0], rev[0])

def get_full_version():
  return Z3_get_full_version()

# Judge if var is an instance of the specific cffi type
def iscffiinstance(var, typestr):
    if type(var) is ffi.CData:
        if typestr in ffi.getctype(ffi.typeof(var)):
            return True
    return False
...
def _get_args(args):
    try:
        if len(args) == 1 and (isinstance(args[0], tuple) or isinstance(args[0], list)):
            return args[0]
        elif len(args) == 1 and (isinstance(args[0], set) or iscffiinstance(args[0], 'Z3_ast_vector')):
...
def _to_sort_ref(s, ctx):
    if __debug__:
        _z3_assert(iscffiinstance(s, 'Z3_sort'), "Z3 Sort expected")
    k = _sort_kind(ctx, s)
    if k == Z3_BOOL_SORT:
...
def _to_expr_ref(a, ctx):
    if iscffiinstance(a, 'Z3_pattern'):
...
def CreateDatatypes(*ds):
    ...
    ctx = ds[0].ctx
    num    = len(ds)
    names  = ffi.new('Z3_symbol[]', num)
    out    = ffi.new('Z3_sort[]', num)
    clists = ffi.new('Z3_constructor_list[]', num)
    to_delete = []
    for i in range(num):
        d        = ds[i]
        names[i] = to_symbol(d.name, ctx)
        num_cs   = len(d.constructors)
        cs       = ffi.new('Z3_constructor[]', num_cs)
        for j in range(num_cs):
            ...
            fs     = c[2]
            num_fs = len(fs)
            fnames = ffi.new('Z3_symbol[]', num_fs)
            sorts  = ffi.new('Z3_sort[]', num_fs)
            refs   = ffi.new("unsigned[]", num_fs)
...
def EnumSort(name, values, ctx=None):
    ...
    ctx = _get_ctx(ctx)
    num = len(values)
    _val_names   = ffi.new('Z3_symbol[]', num)
    for i in range(num):
        _val_names[i] = to_symbol(values[i])
    _values  = ffi.new('Z3_func_decl[]', num)
    _testers = ffi.new('Z3_func_decl[]', num)
    name = to_symbol(name)
...
class ParamDescrsRef:
    ...
    def __init__(self, descr, ctx=None):
        _z3_assert(iscffiinstance(descr, 'Z3_param_descrs'), "parameter description object expected")
        self.ctx    = _get_ctx(ctx)
        self.descr  = descr
...
class Goal(Z3PPObject):
    ...
    def size(self):
        ...
        2
        """
        return Z3_goal_size(self.ctx.ref(), self.goal)
...
class FuncInterp(Z3PPObject):
    ...
    def num_entries(self):
        ...
        3
        """
        return Z3_func_interp_get_num_entries(self.ctx.ref(), self.f)
    ...
    def arity(self):
        ...
        1
        """
        return Z3_func_interp_get_arity(self.ctx.ref(), self.f)
...
class Statistics:
    ...
    def __len__(self):
        ...
        6
        """
        return Z3_stats_size(self.ctx.ref(), self.stats)
...
class Solver(Z3PPObject):
    ...
    def check(self, *assumptions):
        ...
        num = len(assumptions)
        _assumptions = ffi.new('Z3_ast[]', num)
        for i in range(num):
...
class Fixedpoint(Z3PPObject):
    ...
    def sexpr(self):
        """Return a formatted string (in Lisp-like format) with all added constraints. We say the string is in s-expression format.
        """
        return Z3_fixedpoint_to_string(self.ctx.ref(), self.fixedpoint, 0, ffi.NULL)
...
class Tactic:
    ...
    def __init__(self, tactic, ctx=None):
        self.ctx    = _get_ctx(ctx)
        self.tactic = None
        if iscffiinstance(tactic, 'Z3_tactic'):
            self.tactic = tactic
        else:
            if __debug__:
...
class Probe:
    ...
    def __init__(self, probe, ctx=None):
        self.ctx    = _get_ctx(ctx)
        self.probe  = None
        if iscffiinstance(probe, 'Z3_probe'):
            self.probe = probe
...
def substitute(t, *m):
    ...
    num = len(m)
    _from = ffi.new('Z3_ast[]', num)
    _to   = ffi.new('Z3_ast[]', num)
    for i in range(num):
...
def _pb_args_coeffs(args):
    ...
    args = _coerce_expr_list(args, ctx)
    _args, sz = _to_ast_array(args)
    _coeffs = ffi.new('int[]', len(coeffs))
...
def _dict2sarray(sorts, ctx):
    sz = len(sorts)
    _names = ffi.new('Z3_symbol[]', sz)
    _sorts = ffi.new('Z3_sort[]', sz)
    i = 0
    for k in sorts:
        v = sorts[k]
        if __debug__:
...
def _dict2darray(decls, ctx):
    sz = len(decls)
    _names = ffi.new('Z3_symbol[]', sz)
    _decls = ffi.new('Z3_func_decl[]', sz)
    i = 0
    for k in decls:
        v = decls[k]
        if __debug__:
...
def tree_interpolant(pat,p=None,ctx=None):
    ...
    ptr = ffi.new('Z3_ast_vector[1]')
    mptr = ffi.new('Z3_model[1]')
    if p is None:
        p = ParamsRef(ctx)
...
class FPSortRef(SortRef):
    ...
    def ebits(self):
       ...
       8
       """
       return Z3_fpa_get_ebits(self.ctx_ref(), self.ast)
    ...
    def sbits(self):
       ...
       24
       """
       return Z3_fpa_get_sbits(self.ctx_ref(), self.ast)
...
class FPNumRef(FPRef):
    ...
    def sign(self):
        l = ffi.new('int[1]')
        if Z3_fpa_get_numeral_sign(self.ctx.ref(), self.as_ast(), l) == False:
