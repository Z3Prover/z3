...
namespace array {
     ...
     *    n := store(a, i, v)
     */
    void solver::assert_store_axiom1(expr* _e) {
        m_stats.m_num_store_axiom1++;
        SASSERT(a.is_store(_e));
        app* e = to_app(_e);
        ...
        ctx.propagate(n1, n2, store_axiom1());
    }
     ...
     */
    void solver::assert_store_axiom2(expr* _store, expr* _select) {
        m_stats.m_num_store_axiom2a++;
        SASSERT(a.is_store(_store));
        SASSERT(a.is_select(_select));
        app* store = to_app(_store);
        app* select = to_app(_select);
     ...
     *    select(const(v), i) = v
     */
    void solver::assert_select_const_axiom(expr* cnst, expr* _select) {
    ...
    */
    void solver::assert_map(expr* _select, expr* _map) {
        m_stats.m_num_map_axiom++;
        SASSERT(a.is_map(_map));
        SASSERT(a.is_select(_select));        
        app* map = to_app(_map);
        app* select = to_app(_select);
     ...
     */  
    void solver::assert_default_map(expr* _map) {
     ...
     *    default(const(e)) = e
     */  
    void solver::assert_default_const(expr * cnst) {
     ...
     */
    void solver::assert_select_as_array_axiom(expr* _select, expr* arr) {
     ...
     *     default(n) = default(a)
     */
     void solver::assert_default_store_axiom(expr* _store) {
